...
struct Layer
{
  ...
  LayerNeighbour nextLayers[2][5];
  LayerNeighbour prevLayers[2][5];
  ...
  void ReadGeometry( istream & in );
  void WriteGeometry( ostream & out );
};
...
class Geo
{
 public:
  ...
  static constexpr int NLayers = 48;//(7+4+7)+(6+4+6)+(6+2+6); // 48
  ...
  static void ReadLayerGeometry( const char *path="./" );
  static void WriteLayerGeometry( const char *path="./" );
